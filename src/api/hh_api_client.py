...
class Parser(ABC):
    ...
    def search_employers(
        self, name: str, page: int = 0, per_page: int = 20
    ) -> List[Dict]:
        """
...
class ParserConnection(Parser):
    ...
    def search_employers(
        self, names_companies: list, page: int = 0, per_page: int = 20
    ) -> List[Dict]:
        """
        Поиск компаний (работодателей) по имени.
        Возвращает список компаний (self.__data_company).
        """
        for name_company in names_companies:
            url = f"{self.BASE_URL}employers"
            ...
            if resp.status_code != 200:
                raise RuntimeError(
                    f"Ошибка запроса к HH: {resp.status_code} - {resp.text}"
                )
            ...
            if not filtered:
                ...
                if (
                    isinstance(data.get("open_vacancies"), int)
                    and data["open_vacancies"] > 0
                ):
                    filtered = items

            self.__data_company.append(filtered[0])
